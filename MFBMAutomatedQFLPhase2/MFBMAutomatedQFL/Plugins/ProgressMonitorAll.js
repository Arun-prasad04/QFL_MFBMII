...
$(document).ready(function () {
    $("#liProgressMonitorAll").addClass("active");
    InitializeUrl();
...
function ProgressMonitor(plantid) {
    ...
    var ApiFunc = Api + 'Monitor.svc/GetProgressMonitorAllData';
    PostMethod(ApiFunc, Input, Token, function (data) {
        showloader();
                    ...
                    initComplete: function () {
                        this.api().columns().every(function () {
                            ...
                            if (FilterName == "ddlComments" || FilterName == "ddlPhoto") {
                                ...
                                select.append('<option value="ReExam Ok">ReExam Ok</option>')
                                select.append('<option value="ReExam Pending">ReExam Pending</option>')
...
function BindData(data) {
    ...
    $.each(data.listlinedetails, function (i, item) {
        ...
        $.each(secondItem, function (i, item) {
            ...
            $.each(thirdItem, function (i, item) {
                ...
                tablebody += ' <td style="width: 150px;" id="Vinnumber_' + item.VinNumber + '"  class="Vinnumber Vinnumber_' + item.VinNumber + '">' + item.VinNumber + '</td> '; // No header only hidden field for variant and modalid

                tablebody += '<td style="width: 150px;" id="Vehicle_' + item.VinNumber + '"  class="Vehicle Vehicle_' + item.VinNumber + '">' + item.VehicleType + '</td> '; // No header only hidden field for variant and modalid

                tablebody += '<td style="width: 150px;" id="Modal_' + item.VinNumber + '" class="Modal Modal_' + item.VinNumber + '">' + item.ModelName + '</td> '; // No header only hidden field for variant and modalid


                tablebody += '<td   style="width: 250px;" id="PartName_' + item.QFLFeedbackWorkflowId + '"  class="PartName">' + item.inspectionitem + '</td> ';
               // '<div style="display:block" id="colorid_' + item.QFLFeedbackWorkflowId + '"> <div class="square" onclick=GetColour("red")></div> <div class="square1" onclick=GetColour("Yellow")></div> <div class="square2" onclick=GetColour("deepskyblue")></div>  </div> '
                    ...
                    if (item.Site1Image == "") {
                        tablebody += '<td style="width: 250px;" id="Defect_' + item.QFLFeedbackWorkflowId + '" class="Defect">' + item.Defect + '</td>';
                    }
                    else {
                        ...
                        if (item.Defect == "") {
                            ...
                            tablebody += '<td style="width: 250px;" id="Defect_' + item.QFLFeedbackWorkflowId +'" class="Defect">' + '<img style="width:50px" id="" src="' + UserDetails.SignatureSitePath + '/' + item.VinNumber + '/' + item.Site1Image + '" />' + '</td> '; // No header only hidden field for variant and modalid
                            ...
                            tablebody += '<td style="width: 250px;" id="Defect_' + item.QFLFeedbackWorkflowId +'" class="Defect">' + item.Defect + " , " + '<img style="width:50px" id="" src="' + UserDetails.SignatureSitePath + '/' + item.VinNumber + '/' + item.Site1Image + '" />' + '</td> '; // No header only hidden field for variant and modalid
                ...
                if (ReworkCyclecount == 0) {
                    ...
                    if (item.CheckListItemStatusId == 3 || item.CheckListItemStatusId == 6) {
                      ...
                      //  tablebody += ('<span style="display:none">Not Ok</span> </span>');
                       ...
                       // tablebody += ('<span style="display:none">ReExam Pending</span> </span>');
                        tablebody += ('<span style="display:none">ReExam Pending</span>' + '<span style="color: sandybrown;font-weight: bold;">' + ReexaminationCycleCount + '</span>' + '</span>');

                    }
                    else if (item.CheckListItemStatusId == 7) {
                        tablebody += ('<span class="feebback-pending-buttons">');
                        tablebody += ('<img  id="" style="width:28px" src="../Images/ReExamok.png" />');
                        tablebody += ('<span style="display:none">ReExam Ok</span> </span>');
                    }
    ...
    var Height = $('.AutomatedQFLPage-progressmonitorAll-page').height();
    Height = Height - 100;
...
function ExcelExportProgressMonitor(input) {
    ...
    document.location.href = "../Home/DownloadExcelProgressAll?PlantId=" + PlantId + "&VINFrom=" + vinfrom + "&VINTo=" + vinto + "&FromDate=" + Fromdate + "&ToDate=" + Todate;
...
function ProgressMonitorFromandEndDate() {
    ...
    var startDate = new Date(currYear, currMonth, (d.getDate() - 1));
...
function FilterDropdown(i) {
    ...
    //for (var a = 0; a < DynamicddlComlumn.length; a++)
    //{
    //    var FilterName = "ddl"+DynamicddlComlumn[a];

    //    $('#' + FilterName + i).multipleSelect({
    //        filter: true,
    //        placeholder: "Select",
    //        width: "100%",
    //        onClick: function (view) {
    //            arr = [];
    //            view = $("#ddltest1").multipleSelect("getSelects", "text");
    ...
    //            table.column(8+a).search(arr.join('|'), true, false).draw();

    //        },
    //        onUncheckAll: function () {

    //            var table = $('#sorttable' + i).DataTable();
    //            table.column(8 + a).search('').draw();
    ...
    //    $('#' + FilterName).multipleSelect("uncheckAll");
    //}
